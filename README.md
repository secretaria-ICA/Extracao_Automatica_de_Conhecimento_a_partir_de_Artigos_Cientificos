...
A análise textual semântica visa examinar a similaridade entre dois textos ou trechos de texto. Para tal, nós utilizamos três abordagens diferentes: [Word2Vec](https://radimrehurek.com/gensim/models/word2vec.html) e [Doc2Vec](https://radimrehurek.com/gensim/models/doc2vec.html) para a criação de representações vetoriais numéricas (ou *word/document embeddings*, que se referem aos pesos dos neurônios da camada densa) de palavras e parágrafos/documentos, respectivamente, de modo a encontrar as relações entre as palavras. Para a visualização interativa do modelo de tópico e termos mais frequentes, nós empregamos a [Alocação Latente de Dirichlet (LDA)](https://github.com/bmabey/pyLDAvis). O modelo treinado Word2Vec-CBOW (i.e. *Continuous Bag of Words Model*) foi bem sucedido em identificar palavras similares e dissimilares aos termos avaliados, como "gdf11" e "miostatina". Apesar disso, a avaliação das cinco palavras (top5) mais similares revelou que os termos #3 a #5 não eram próximos ao termo da busca. Por sua a vez, a utilização de uma lista de palavras aleatórias mostrou que o modelo gerado produziu resultados satisfatórios. A avaliação do modelo contra a coleção teste [wordsim353](http://alfonseca.org/eng/research/wordsim353.html) produziu um coeficiente de correlação de Pearson de 0.0925 e um coeficiente de correlação de Spearman de 0.0929, indicando que as variáveis não dependeram linearmente uma da outra. Já a visualização do gráfico de t-SNE confirmou que o modelo treinado conseguiu separar bem as palavras similares ao termo "gdf11" das palavras não similares. Já o modelo treinado Doc2Vec-DM (i.e. *distributed memory*, que preserva a ordem das palavras no documento) também foi capaz de identificar os artigos que possuíam trechos mais semelhantes. Por exemplo, quando a frase "*Loss of myostatin increases muscle mass.*" foi utilizada como busca, o artigo intitulado "*The versatility and paradox of GDF 11*" ([Jamaiyar et al. 2017](https://linkinghub.elsevier.com/retrieve/pii/S0163-7258(17)30046-3)) foi recuperado. Nele, foi possível encontrar a frase "*Myostatin-null mice, cattle and dogs exhibit extreme muscle hypertrophy*". Por fim, a visualização dos tópicos mais frequentes no modelo LDA revelou que o pré-processamento e a limpeza do conjunto de documentos precisam ser refeitos minuciosamente, uma vez que muitos termos sem significado ainda estavam presentes. Por essa razão, não foi possível extrair conhecimento dessa análise no momento.
